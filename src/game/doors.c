...
void InitAccess(void) { acs_level=0; }
...
{
    unsigned cells=(unsigned)(maze_w+2u)*(unsigned)(maze_h+2u);
    unsigned i,count=0;
...
void SaveDoors(void)
{
    ...
    cells=(unsigned)(maze_w+2u)*(unsigned)(maze_h+2u);
    pos=0;
    for (i=0;i<cells;++i) {
...
{
    size_t row=(size_t)(maze_w+2u)*CELL;
...
void DoorKeys(void)
{
    ...
    gx=x_pos>>16;gy=y_pos>>16;if(gx>=(size_t)(maze_w+2u)||gy>=(size_t)(maze_h+2u))return;
    rowidx=gy*(size_t)(maze_w+2u)+gx;cell=maze_data+rowidx*CELL;side=facing_side();panelp=cell+side;panel=*panelp;
    ...
    if((flags&D2_SPECIAL)==D2_SPECIAL){
        ptrdiff_t step;size_t row=(size_t)(maze_w+2u)*CELL;
